...
void ellipse::setup()
{
	...
	vertexPositions.push_back(glm::vec4(0, 0.5f, 0.0f, 1.0f));
	vertexPositions.push_back(glm::vec4(0.2f, 0.49f, 0.0f, 1.0f));
	vertexPositions.push_back(glm::vec4(0.6f, 0.4f, 0.f, 1.0f));
	vertexPositions.push_back(glm::vec4(1.0f, 0.0f, 0.f, 1.0f));
	vertexPositions.push_back(glm::vec4(0.6f, -0.4f, 0.f, 1.0f));
	vertexPositions.push_back(glm::vec4(0.2f, -0.49f, 0.0f, 1.0f));
	vertexPositions.push_back(glm::vec4(0, -0.5f, 0.0f, 1.0f));//
	vertexPositions.push_back(glm::vec4(-0.2f, -0.49f, 0.0f, 1.0f));
	...
	for (int i = 0; i < 9; i++)
	{
		mVertices.push_back(vertexPositions[0]);
		mVertices.push_back(vertexPositions[i+1]);
		mVertices.push_back(vertexPositions[i + 2]);
	}
	...
	if (mType == 0)
	{
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 4 * mVertices.size(), mVertices.data(), GL_STATIC_DRAW);
...
void ellipse::Draw(glm::mat4 & model, glm::mat4 & view, glm::mat4 & projection)
{
	...
	glBindVertexArray(vaoHandle);
	glDrawArrays(GL_TRIANGLES, 0, 4 * mVertices.size());
...
void ellipse::Draw(glm::mat4 & model, glm::mat4 & view, glm::mat4 & projection, glm::mat4 &offset, int colorNum)
{
	glm::mat4 mview = view * model;
	glm::mat4 mvp = projection * view * model;
