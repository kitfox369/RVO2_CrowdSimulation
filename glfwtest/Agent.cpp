...
namespace RVO {
	Agent::Agent(RVOSimulator *sim) : maxNeighbors_(0), maxSpeed_(10000.0f), neighborDist_(0.0f), radius_(0.0f), sim_(sim), timeHorizon_(0.0f), timeHorizonObst_(0.0f), id_(0) { }
	...
	void Agent::computeNewVelocityE()
	{
		orcaLines_.clear();
		...
		for (size_t i = 0; i < agentNeighbors_.size(); ++i) {
			...
			if (distSq > combinedRadiusSq) {
				...
				float a = 2.0f;
				float b = 1.0f;
				float denominator = ((3*b)/(2*(a*a)))*relativePosition*relativePosition;
				float numerator = a*a*pow(1- (relativeVelocity * relativeVelocity / (a*a)), sim_->powfScalar);
				u = denominator / numerator * relativeVelocity;

				//std::cout << "agent powfScalar:"<< sim_->powfScalar << std::endl;
			}

			line.point = velocity_ + 0.5f * u;

			float angle = glm::acos(glm::dot(glm::vec2(velocity_.x(), velocity_.y()),glm::vec2(u.x(),u.y())));
			//float angle = glm::acos(glm::dot(glm::vec2(relativeVelocity.x(), relativeVelocity.y()), glm::vec2(u.x(), u.y())));
			...
			//vector 라인을 회전시켜야함 (u랑 velocity랑 사이각 구한 후 그 각만큼 회전)
			if (distSq < combinedRadiusSq) //충돌할때만 계산
			{
			//	std::cout << angle << std::endl;

				glm::vec2 rotateLine;
				rotateLine.x = (line.direction.x()*glm::cos(angle) - line.direction.y()*glm::sin(angle));
				rotateLine.y = (line.direction.x()*glm::sin(angle) + line.direction.y()*glm::cos(angle));

				line.direction = RVO::Vector2(rotateLine.x, rotateLine.y);
			}
